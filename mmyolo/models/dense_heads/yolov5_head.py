...
class YOLOv5HeadModule(BaseModule):
    ...
    def __init__(self,
        ...
        #print(self.num_classes,self.num_base_priors,self.num_out_attrib)
...
class YOLOv5Head(BaseDenseHead):
    ...
    def predict_by_feat(self,
        ...
        """
        #print("in predict_by_feat", len(cls_scores), cls_scores[0].shape, cls_scores[1].shape, cls_scores[2].shape)
        #print(len(bbox_preds), bbox_preds[0].shape)
        #print(len(objectnesses), objectnesses[0].shape,objectnesses[1].shape,objectnesses[2].shape)
        ...
        #print(len(flatten_cls_scores),flatten_cls_scores[0].shape, flatten_bbox_preds[0].shape)
        ...
        for (bboxes, scores, objectness,
            ...
            ####lin: find the instance idx
            
            detached_cls_scores = cls_scores[1][im_cnt].detach().sigmoid()#get the middle feature output
            reduced_cls_scores,_ = torch.max(detached_cls_scores, 0)
            #print(reduced_cls_scores.shape,reduced_cls_scores[:5,:5])#()
            detached_objectnesses = objectnesses[1][im_cnt].detach().sigmoid()
            reduced_objectnesses,_ = torch.max(detached_objectnesses, 0)
            #print(reduced_objectnesses.shape,reduced_objectnesses[:5,:5])
            instance_scor=torch.mul(reduced_cls_scores, reduced_objectnesses)
            #print(instance_scor.shape, instance_scor[:5,:5])
            instance_idx=torch.argmax(instance_scor).item()
            #print(instance_idx)
            ins_size=instance_scor.shape[0]
            ins_row, ins_col=divmod(instance_idx, ins_size)
            #print(ins_row,ins_col,instance_scor[ins_row,ins_col])
            im_cnt=im_cnt+1
            ...
            if 'pad_param' in img_meta:
                ...
                pad_param = None
            #print(len(scores),len(objectness))
            #print(scores)
            #print(objectness)
            ...
            results['instance_row_idx']=torch.tensor([ins_row] * len(labels)).cuda()
            results['instance_col_idx']=torch.tensor([ins_col] * len(labels)).cuda()
            #print(results)
            if rescale:
